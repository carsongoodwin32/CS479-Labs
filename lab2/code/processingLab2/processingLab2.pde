...
int[] allA1 = new int[100000]; // Array to store all analogRead(A1) values
int[] allA3 = new int[100000]; // Array to store all analogRead(A3) values
...
void drawHRBR() {
  ...
  if(numSamples>=25){
    if(numSamples%25==0){//calcHR
      calculateHR();
    }
    if(numSamples%150==0){//calcBR
      calculateBR();
    }
  }
  else{
  ...
  fill(0, 255, 0);
  textSize(32);
  text("Current Heart Rate: "+curHR, 20, 30);
  text("Current Breathing Rate: "+curBR, 20, 70);
}
...
void calculateHR() {
  ...
  for(int i = numSamples-25; i<numSamples; i++){//for the whole array.
    if(allA3[i]>0){//don't skew data with 0's
      arrayAvg += allA3[i];//Add up entire array
    }
  }
  if(arrayAvg>0){//make sure that the average of the whole array is more than 0, if not, we probably don't have any data.
    arrayAvg = arrayAvg/25;//get average of all values in array.
    ...
    for(int i = numSamples-25; i<numSamples; i++){//for the whole array.
      if(allA3[i]>=arrayAvg*1.2&&allowPeak){//if current value is above the average, indicating a peak, and a peak can be logged.
        numPeaks+=1;//add 1 to total number of peaks.
      }
 
      if(allA3[i]<=arrayAvg*.9&&!allowPeak){//if current value goes lower than our target peak value.
        allowPeak = true;// allow another peak to be recorded.
      }
    }
      curHR = (numPeaks/5)*60;//numPeaks in 30s, multiply by 2 to get 60s.
    }
  else{
  }
...
void calculateBR() {
  int arrayAvg = 0; // average of all values in the calibrate A3 
  for(int i = numSamples-150; i<numSamples; i++){//for the whole array.
    if(allA1[i]>0){//don't skew data with 0's
      arrayAvg += allA1[i];//Add up entire array
    }
  }
  if(arrayAvg>0){//make sure that the average of the whole array is more than 0, if not, we probably don't have any data.
    ...
    boolean allowPeak = true; //only get one measurement of the same peak. disable adding a peak until we go closer to the average.
    for(int i = numSamples-150; i<numSamples; i++){//for the whole array.
      if(allA1[i]<=arrayAvg&&!allowPeak){//if current value goes lower than our target peak value.
        allowPeak = true;// allow another peak to be recorded.
      }
      if(allA1[i]>=arrayAvg*1.5&&allowPeak){//if current value is above the average, indicating a peak, and a peak can be logged.
        numPeaks+=1;//add 1 to total number of peaks.
        allowPeak = false;
      }
    }
    curBR = ((numPeaks)*2);//return numPeaks*2 because num peaks in 30s *2 gets breathing rate per minute.
  }
  else{
  }
}
...
void calculateBSHR() {
  ...
  if(arrayAvg>0){//make sure that the average of the whole array is more than 0, if not, we probably don't have any data.
    ...
    boolean allowPeak = true; //only get one measurement of the same peak. disable adding a peak until we go closer to the average.
    for(int i = 0; i<150; i++){//for the whole array.
      if(calibA3[i]>=arrayAvg*1.1&&allowPeak){//if current value is above the average, indicating a peak, and a peak can be logged.
...
void serialEvent(Serial port) {
  ...
  if (data != null) {
    ...
    if (values.length == 2 && logToArrays && !beginCalib) {
      ...
      allA1[numSamples] = valueA1;
      allA3[numSamples] = valueA3;
      numSamples+=1;
